...
#' Point mapping
#'
#' A function predicting the VWC at specific points, with resolution of 30.
#' Rocommended: date range less than 6 months.
#'
#' @param file character. Gives the name of the file that stores the point locations (in csv format). It can be a file within the working directory or a detailed path specifying the location of the file.
...
#' @param project character. Defines the name of the folder to be created and save the output csv. If not defined, the maps and csv file will be saved in the working directory.
...
#'
#' @return a data frame of the predicted VWC values, corresponding to the point locations.
#'
#' @import FedData rgee randomForest quantregForest caret sp sf raster rgdal leaflet RColorBrewer tidyverse reshape2
#'
#' @export
#'
#' @examples
#' \dontrun{ VWC_point("WI_SM.csv", "2020-06-15", "2020-09-15", TRUE, "WI_point") }
VWC_point <- function(file, start_date, end_date, percentile=FALSE,
                     project=NULL, filename="VWC_point_data.csv"){
  ...
  if(!is.null(project)){
    if (file.exists(project)){
      ...
      dir.create(project)
      root_WD <- paste0(WD, "/", project)
    }
  }

  bunk <- function(site, current, total, i, message, pb, j){
    cat("0\14")
    writeLines(c(paste0("Site: ", site, " (",current,"/",total,")"),paste0("Tasks done: ", i, "/10"),message))
  ...
  ############################ Part 2  Mapping soil moisture at surface (0-5 cm) and rootzone (0-1 m)
  {
    ...
    sites <-  read.csv(file, header=T)
    total <- nrow(sites)

    print(paste0("Sites = ", file))
    ...
    for (m in 1:total){
      ...
      msg <- paste0("Site: ", id, " (",m,"/",total,")")
      writeLines(msg)

      dir.create(paste0(root_WD, "/", subsite$ID))
      WD <- paste0(root_WD, "/", subsite$ID)

      roi <- ee$Geometry$Point(list(as.numeric(subsite$Longitude), as.numeric(subsite$Latitude)))$buffer(radius)
      ...
      ## Input region, resolution, and dates
      {
        ## Define input region of interest (ROI)

        ## 1) Get the NRCS SSURGO data (USA ONLY)
        # roi_name <- "IL"
        # boundary <- readOGR(WD, roi_name)
        # SSURGO.VEPIIN <- get_ssurgo(template = boundary, label='ROI')

        ## Plot the SSURGO mapunit polygons
        # plot(boundary)
        # plot(SSURGO.VEPIIN$spatial, lwd=0.1, add=T)
        # area(boundary)/1000000 # in km2

        ## 2) Input ROI directly
        # bound <- st_read(WD, roi_name)
        # roi <- sf_as_ee(bound$geometry)

        ## 3) Input ROI manually
        # roi <- ee$Geometry$Polygon(list(
        #   c(-89.12001886714096, 42.5697123128558),
        #   c(-89.11027708399887, 42.56958589188363),
        #   c(-89.11014833796615, 42.58197392928661),
        #   c(-89.11263742793197, 42.58162633934683),
        #   c(-89.11405363429184, 42.57751831146452),
        #   c(-89.11499777186508, 42.57726550090856),
        #   c(-89.11735811579818, 42.57773951986022),
        #   c(-89.12010469782943, 42.57773951986022)
        # ))
      ...
      ## Download land surface parameters from GEE and Cloud Storage
      {
        ...
        LC = ee$ImageCollection("USGS/NLCD")$filterDate('2016-01-01', '2016-12-31')$select('landcover')$first()$rename("LC")$reproject(crs = "EPSG:4326", scale = resolution)$clip(roi)

        cat("\014")
        writeLines(c(msg,"Tasks done: 0/10","Downloading static covariates..."))
        ...
        bunk(id, m, total, 0,"Downloading static covariates...",pb,1)
        ...
        bunk(id, m, total, 0,"Downloading static covariates...",pb,3)
        ...
        bunk(id, m, total, 0,"Downloading static covariates...",pb,4)
        close(pb)
      ...
      {
        cat("\014")
        writeLines(c(msg, "Tasks done: 6/10","Temporal gap-filling..."))
        ...
        bunk(id, m, total, 6,"Temporal gap-filling...",pb,1)
        ...
        if(!vv_na)
        {
          ...
          bunk(id, m, total, 6,"Temporal gap-filling...",pb,4)
          ...
          bunk(id, m, total, 6,"Temporal gap-filling...",pb,5)
          ...
          bunk(id, m, total, 6,"Temporal gap-filling...",pb,6)
        ...
        bunk(id, m, total, 6,"Temporal gap-filling...",pb,7)
        ...
        bunk(id, m, total, 6,"Temporal gap-filling...",pb,8)
        ...
        bunk(id, m, total, 6,"Temporal gap-filling...",pb,9)
        ...
        bunk(id, m, total, 6,"Temporal gap-filling...",pb,10)
        ...
        bunk(id, m, total, 6,"Temporal gap-filling...",pb,11)
        close(pb)
        ...
        # cat("\014")
        # writeLines(c(msg, "Tasks done: 7/10","Filling NAs..."))
        # pb <- txtProgressBar(min=0, max=dim(LS_B4_raster)[3], style=3)
        # for (i in 1:dim(LS_B4_raster)[3])
        # {
        ...
        #   bunk(id, m, total, 7,"Filling NAs...",pb,i)
        # }
        # close(pb)

        cat("\014")
        writeLines(c(msg, "Tasks done: 7/10","Calculating NDWI..."))
        ...
        bunk(id, m, total, 7,"Calculating NDVI...",pb,i)
      ...
      ## Create folder to store downloaded covariates
      {
        ...
        cat("\014")
        writeLines(c(msg, "Tasks done: 8/10","Saving covariates..."))
      ...
      {
        VWC_5_mean_pts <- VWC_5_sd_pts <- VWC_100_mean_pts <- VWC_100_sd_pts <- NULL

        if(percentile){
          VWC_5_lower_pts <- VWC_5_upper_pts <- VWC_100_lower_pts <- VWC_100_upper_pts <- NULL
        }

        ## For each date, apply the established ML model (reduced model) to the covariate rasters
        cat("\014")
        writeLines(c(msg, "Tasks done: 9/10","Applying ML model..."))
        ...
        for (i in 1:length(dates))
        {
          ...
          VWC_5_mean_pts <- c(VWC_5_mean_pts, round(mean( grid_VWC_5_mean, na.rm = T), digits=3))
          VWC_5_sd_pts <- c(VWC_5_sd_pts, round(mean( grid_VWC_5_sd, na.rm = T), digits=3))
          VWC_100_mean_pts <- c(VWC_100_mean_pts, round(mean( grid_VWC_100_mean, na.rm = T), digits=3))
          VWC_100_sd_pts <- c(VWC_100_sd_pts, round(mean( grid_VWC_100_sd, na.rm = T), digits=3))
          ...
          if(percentile!=FALSE){
            ...
            VWC_5_lower_pts <- c(VWC_5_lower_pts, round(mean( grid_VWC_5_lower, na.rm = T), digits=3))
            VWC_5_upper_pts <- c(VWC_5_upper_pts, round(mean( grid_VWC_5_upper, na.rm = T), digits=3))
            VWC_100_lower_pts <- c(VWC_100_lower_pts, round(mean( grid_VWC_100_lower, na.rm = T), digits=3))
            VWC_100_upper_pts <- c(VWC_100_upper_pts, round(mean( grid_VWC_5_upper, na.rm = T), digits=3))
          }

          if(i<length(dates)) bunk(id, m, total, 9,"Applying ML model...",pb,i)
          if(i==length(dates)) bunk(id, m, total, 10,"Applying ML model...",pb,i)
        }
        close(pb)

        out_temp <- cbind(VWC_5_mean_pts, VWC_5_sd_pts, VWC_100_mean_pts, VWC_100_sd_pts)
        if (percentile) out_temp <- cbind(out_temp, VWC_5_lower_pts, VWC_5_upper_pts,
                                                VWC_100_lower_pts, VWC_100_upper_pts)
        out_temp <- as.data.frame(out_temp)
        out_temp$Date <-  dates
        out_temp$ID  <-  subsite$ID
        out_temp$Longitude <- subsite$Longitude
        out_temp$Latitude <- subsite$Latitude

        out_temp <- dplyr::select(.data=out_temp, ID, Longitude, Latitude, Date, everything())
        write.csv(out_temp, paste0(WD, "/", filename), row.names=F)

        out_all <- rbind(out_all, out_temp)
      }
  ...
  # out_all <- select(.data=out_all, ID, Longitude, Latitude, Date, everything())
  write.csv(out_all, paste0(root_WD, "/", filename), row.names=F)
